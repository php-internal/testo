...
use Testo\Module\Tokenizer\Reflection\TokenizedInvocation;
 ...
 */
final class InvocationLocator extends FileLocator implements InvocationsInterface
{
     ...
     * @return \Generator<int, TokenizedInvocation>
     */
    protected function availableInvocations(string $signature = ''): \Generator
    {
        ...
        foreach ($this->availableReflections() as $reflection) {
            foreach ($reflection->getInvocations() as $invocation) {
                if (
                    !empty($signature)
                    && \strtolower(\trim($invocation->name, '\\')) !== $signature
                ) {
                    continue;
                }
    ...
    protected function isTargeted(TokenizedInvocation $invocation, \ReflectionFunctionAbstract $function): bool
    {
        ...
        try {
            $reflection = $this->classReflection($invocation->class);
        ...
        if ($target->isTrait()) {
            ...
            return \in_array($target->getName(), $this->fetchTraits($invocation->class));
        }
